...
class TestSummarizeRoute:
    ...
    def test_timing_stats(self):
        records = [
            _make_record(
                "single_call", _base_metrics(), timing={"full_run_time_seconds": 1.0, "n_api_calls": 1}
            ),
            _make_record(
                "single_call", _base_metrics(), timing={"full_run_time_seconds": 2.0, "n_api_calls": 1}
            ),
            _make_record(
                "single_call", _base_metrics(), timing={"full_run_time_seconds": 3.0, "n_api_calls": 1}
            ),
        ]
        summary = _summarize_route(records)
...
class TestReportGeneration:
    ...
    def test_order_variant_section_present(self, tmp_path):
        records = [
            _make_record(
                "single_call_O1_late_screen_context", _base_metrics(), order_variant="late_screen_context"
            ),
        ]
        report_path = generate_report(records, _MINIMAL_CONFIG, tmp_path)
        text = report_path.read_text(encoding="utf-8")
