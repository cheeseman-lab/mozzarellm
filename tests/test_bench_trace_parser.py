...
Inline-MCP route tests (3a_mcp, 3a_mcp_O3_early_output_format) were removed
when MCP became the single_call_mcp enrichment; the underscore-heavy-name
greedy-match coverage they provided is preserved below using single_call_mcp,
the still-live mode with the same naming shape.
"""
...
def _make_meta(**overrides) -> dict:
    base = {
        ...
        "route": "single_call",
        "replicate": 1,
        "run_id": "exp__single_call__denali__cluster_21__rep_1",
    }
...
def _make_trace_json(
    *,
    route: str = "single_call",
    run_id: str = "exp__single_call__denali__cluster_21__rep_1",
...
class TestFilenameParser:
    def test_phase1_standard_route(self):
        result = parse_trace_filename(Path("exp__single_call__denali__cluster_21__rep_1.json"))
        assert result["experiment_id"] == "exp"
        assert result["route"] == "single_call"
        assert result["screen_name"] == "denali"
        assert result["cluster_id"] == "21"
        assert result["replicate"] == 1
        assert result["filename_parse_status"] == "success"

    def test_phase1_codegap_route(self):
        result = parse_trace_filename(
            Path("exp__single_call_mcp__denali__cluster_21__rep_1.json")
        )
        assert result["route"] == "single_call_mcp"
        assert result["filename_parse_status"] == "success"

    def test_phase1_stepwise(self):
        result = parse_trace_filename(Path("exp__stepwise__screen__cluster_42__rep_3.json"))
        assert result["route"] == "stepwise"
        assert result["replicate"] == 3
        assert result["filename_parse_status"] == "success"

    def test_phase2_order_variant(self):
        result = parse_trace_filename(
            Path("exp__cot_O1_late_screen_context__denali__cluster_21__rep_1.json")
        )
        assert result["route"] == "cot_O1_late_screen_context"
        assert result["filename_parse_status"] == "success"

    def test_phase2_codegap_order_variant(self):
        result = parse_trace_filename(
            Path("exp__single_call_mcp_O3_early_output_format__denali__cluster_77__rep_2.json")
        )
        assert result["route"] == "single_call_mcp_O3_early_output_format"
    ...
    def test_phase3_wording_route(self):
        result = parse_trace_filename(
            Path(
                "exp__single_call_wording_v1_W3_imperative_gene_classification"
                "__denali__cluster_21__rep_1.json"
            )
        )
        assert result["filename_parse_status"] == "success"
        assert result["route"] == "single_call_wording_v1_W3_imperative_gene_classification"

    def test_underscore_heavy_screen_name(self):
        result = parse_trace_filename(
            Path("exp__single_call__aconcagua_interphase_shuffled__cluster_244__rep_1.json")
        )
    ...
    def test_multi_digit_cluster_id(self):
        result = parse_trace_filename(Path("exp__cot__denali__cluster_109__rep_1.json"))
...
class TestEndToEnd:
    ...
    def test_extract_and_write_csvs(self, tmp_path: Path):
        ...
        self._write_trace(
            traces_dir,
            "exp__single_call__denali__cluster_21__rep_1.json",
            _make_trace_json(
                route="single_call",
                run_id="exp__single_call__denali__cluster_21__rep_1",
        ...
        self._write_trace(
            traces_dir,
            "exp__single_call__denali__cluster_22__rep_1.json",
            _make_trace_json(
                route="single_call",
                run_id="exp__single_call__denali__cluster_22__rep_1",
                established_genes=["G3"],
            ),
        )
        self._write_trace(
            traces_dir,
            "exp__cot__denali__cluster_21__rep_1.json",
            _make_trace_json(
                route="cot",
                run_id="exp__cot__denali__cluster_21__rep_1",
                established_genes=["G4"],
            ),
        )

        route_rows = extract_predictions_from_traces(traces_dir)
        assert "single_call" in route_rows
        assert "cot" in route_rows
        assert len(route_rows["single_call"]) == 3
        assert len(route_rows["cot"]) == 1
        ...
        for csv_path in written:
            with open(csv_path, newline="", encoding="utf-8") as f:
                ...
                rows = list(reader)
                if "single_call" in csv_path.name:
                    assert len(rows) == 3
                else:
                    assert len(rows) == 1
    ...
    def test_trace_missing_parsed_output(self, tmp_path: Path):
        ...
        route_rows = extract_predictions_from_traces(traces_dir)
        assert "single_call" in route_rows
        assert len(route_rows["single_call"]) == 1

    def test_trace_invalid_json(self, tmp_path: Path):
        traces_dir = tmp_path / "traces"
        traces_dir.mkdir()

        bad = traces_dir / "exp__single_call__denali__cluster_21__rep_1.json"
