...
def server_model_chat(context, prompt, model, temperature, max_tokens, log_file, seed=None):
    """
    ...
    if ":" in model:
        ...
        server_url = os.environ.get("MODEL_SERVER_URL", "http://localhost:8000/v1/chat/completions")
        model_name = model
    ...
    for attempt in range(max_retries):
        try:
            response = requests.post(server_url, headers=headers, json=data, timeout=120)
