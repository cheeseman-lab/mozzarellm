...
STRENGTH_COL = "phenotypic_strength"
...
def augment_phenotype(bundles_dir: Path = OUTPUT_DIR, benchmark_csv: Path = BENCHMARK_CSV) -> int:
    ...
    the per-gene phenotypic_strength values replaced by "N/M" ranks (ranked per
    screen over the benchmark's scored genes -- M is the benchmark's gene set
    for that screen, not the full screen) plus the cluster-level
    phenotype_strength table. Raw strength never reaches the model. Annotation
    ...
    for screen_name, screen_df in df.groupby(SCREEN_COL):
        if screen_df[STRENGTH_COL].notna().any():
            ranked = attach_strength_ranks(screen_df, STRENGTH_COL)
            for _, row in ranked.iterrows():
                if isinstance(row[STRENGTH_COL], str):
                    ranks[(screen_name, row[GENE_COL])] = row[STRENGTH_COL]
    ...
    for path in sorted(bundles_dir.glob("*__bundle.json")):
        ...
        for gene in genes:
            gene.pop(STRENGTH_COL, None)
            rank = ranks.get((bundle["screen_name"], gene[GENE_COL]))
            if rank:
                gene[STRENGTH_COL] = rank
        ...
        if STRENGTH_COL in chunk.columns:
            bundle["phenotype_strength"] = compute_phenotype_strength(
                chunk, gene_column=GENE_COL, strength_column=STRENGTH_COL
            )
