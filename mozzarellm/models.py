...
class ClusterResult(BaseModel):
    ...
    established_genes: list[str] = Field(
        default_factory=list, description="Well-known members of the identified pathway"
    )
    ...
    novel_role_genes: list[GeneClassification] = Field(
        default_factory=list, description="Genes with known functions in other pathways"
    )
    summary: str = Field(..., description="Concise summary of key findings")
    raw_response: str | None = Field(None, description="Raw LLM response text")

    # Quality metrics
    missed_genes: list[str] = Field(
        default_factory=list, description="Genes in cluster that were not classified by LLM"
    )
    total_genes_in_cluster: int = Field(
        default=0, description="Total number of genes in the input cluster"
    )
...
class AnalysisResult(BaseModel):
    ...
    def get_all_high_confidence_clusters(self) -> list[ClusterResult]:
        ...
        return [
            cluster for cluster in self.clusters.values() if cluster.pathway_confidence == "High"
        ]

    def get_total_flagged_genes(self) -> int:
        """Count total flagged genes across all clusters."""
        return sum(len(cluster.get_all_flagged_genes()) for cluster in self.clusters.values())
...
class ClusterInput(BaseModel):
    ...
    gene_annotations: dict[str, str] | None = Field(
        None, description="Optional annotations for genes (gene -> annotation text)"
    )
...
class RetrievalContext(BaseModel):
    ...
    snippets: list[dict] = Field(
        default_factory=list, description="Evidence snippets with text, source, and relevance score"
    )
    citations: list[dict] = Field(
        default_factory=list, description="Citation information for sources"
    )
    retrieval_metadata: dict = Field(
        default_factory=dict, description="Metadata about the retrieval process"
    )
